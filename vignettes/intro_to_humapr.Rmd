...
## Getting started
`humapr` for now isn't on available on CRAN, so using `install.packages` won't get you far. Instead, use the `devtools` package to fetch `humapr` from GitHub where it currenly lives. If you don't have it, run `install.packages("devtools")`. My current R version is outdates, but this doesn't really cause problems. 
