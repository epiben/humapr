...
It shouldn't be a big surprised that this geom maps your data onto the stylised body surface. We first simulate some to play with:
```{r}
body_data <- data.frame(loc = sampler(c("head", "neck", "chest", "abdomen", "arm", "forearm", "hand", "thigh", "leg", 
                                        "foot"), 200),
                        side = sampler(c("left", "right"), 200),
                        gender = sample(c("male", "female"), 200, TRUE),
                        year = 2010:2019) 
knitr::kable(sample_n(body_data, 10), align = "c")
```
...
Again, we first create some simulated data:

```{r}
organs <- c("urogenital", "right_kidney", "left_kidney", "pancreas", "liver", "spleen", "oesophagus", "stomach", 
            "intestine", "heart", "trachea", "hyoid", "larynx", "right_lung", "left_lung")
organ_data <- data.frame(loc = sampler(organs, 300),
                         gender = sampler(c("Female", "Male"), 300),
                         period = sampler(c("2001-05", "2006-10"), 300))
```

Then, we make a simple tabulation plot of the observation in each organ. Because organs, generally, are not left-right symmetric, there's no `side` argument in `geom_organs`. Instead, you assigned observations directly to, e.g., `right_lung` and `left_kidney`.
```{r}
humap() +
